...
### 10.31 금
- [ ] 경로 하드코딩을 os로 변경
- [ ] db 새로 만들기 (병합 안되는 것 같다) + 폴더에 새 기사 들어오면 자동추가되게 구현
- [ ] '동향'이라는 단어 들어가면 무조건 최신 기사만 반영하도록 하기 (단일 DB 전제로. 날짜 설정.)
- [ ] stg~~.com (demo 말고) 으로 접속 되게..?
- [ ] DB 하나로 합치기
- [ ] os 사용해서 운영/개발 서버 경로 반영되게
- [ ] 서버 인식해서 api 번호 결정
- [ ] 관련기사 개수 안나오게 해달라고 요청 드리고 **요약에 반영된 것만 마지막에 나오게 수정**
- [ ] 경기도뉴스 API
- [ ] DB에 ky_news_recent 자동 추가되게
- [ ] searxng, pv 구현
- [ ] ky_news_recent 폴더에 최신기사 전달되고 있는데 개발용 stg_ky_news_recent 폴더 경로도 말씀드리기
- [ ] 로그 한달 단위로 백업 및 backend.log 초기화되게 구현
- [ ] 너 누구야 이해 못하는 원인 찾기
